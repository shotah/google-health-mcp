# google-health-mcp Makefile — same vibes as go-garmin / cars-search-mcp
#
...
LDFLAGS := -s -w -X main.version=$(VERSION) -X github.com/shotah/google-health-mcp/internal/mcp.ServerVersion=$(VERSION)

BUMP ?= patch
PKG ?= ./...

BINARY ?= bin/google-health-mcp
ifeq ($(OS),Windows_NT)
BINARY := bin/google-health-mcp.exe
endif
...
help: ## Show this help
	...
	@echo   install                Install google-health-mcp into GOPATH/bin
...
cli: ## Build static MCP binary into ./bin/
	mkdir -p bin
	CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o $(BINARY) ./cmd/google-health-mcp

install: ## Install into GOPATH/bin
	CGO_ENABLED=0 go install -trimpath -ldflags "$(LDFLAGS)" ./cmd/google-health-mcp

run: ## go run MCP — e.g. make run ARGS="--self-test"
	go run -ldflags "$(LDFLAGS)" ./cmd/google-health-mcp $(ARGS)
...
clean: ## Remove binaries and coverage
	go clean ./...
ifeq ($(OS),Windows_NT)
	-cmd /C "rmdir /S /Q bin 2>NUL & del /Q coverage coverage.out coverage.txt google-health-mcp.exe 2>NUL"
else
	rm -rf bin
	rm -f coverage coverage.out coverage.txt google-health-mcp google-health-mcp.exe
endif
